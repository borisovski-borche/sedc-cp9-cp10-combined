const DataService = {
    ...
    fetchQuizzes: async function () {
        try {
            let response = await fetch(`https://github.com/borisovski-borche/sedc-cp9-cp10-combined/blob/combined-folder-structure/public/DigitalCommunicator/JSON/Quizzes.json`);
            let data = await response.json();
            DataService.cachedQuizzes = await data;
        } catch (error) {
            QuizzesService.gamesAndQuizzesWindow.innerHTML =
                `<div id="errorWrapper" class="errorWrapper"> <img src="./DigitalCommunicator/src/img-avatars/quizzesError.png" alt="Error Img">
                <div class="errorText">"Oops, Something is wrong with my circuits I can't process the command"</br>Please try again later!</div>
            </div>`;
        }
    },
